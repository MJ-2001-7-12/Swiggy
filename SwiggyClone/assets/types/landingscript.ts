// Define types for your variables if needed
interface PlaceName {
    name: {
      first: string;
    };
  ...
  // Changing headings
  const changingHeading = document.getElementById("changingHeading") as HTMLHeadingElement;
  const texts: string[] = [
  ...
  // Legal section
  const legalList = document.getElementById("legalList") as HTMLUListElement;
  ...
  fetch(`https://randomuser.me/api/?results=${numberOfPlaces}`)
    ...
    .then((data) => {
      const placeNames: string[] = data.results.map((result: PlaceName) => result.name.first);
      console.log(placeNames);
      let y = 0;
      for (let i = 1; i <= 4; i++) {
        const weDeliverTo = document.getElementById(`wedeliverList${i}`) as HTMLUListElement;
        for (y; y < i * 25; y++) {
      ...
      y = 0;
      for (let i = 1; i <= 4; i++) {
        const bestPlaces = document.getElementById(`bestPlacesList${i}`) as HTMLUListElement;
        for (y; y < i * 25; y++) {
